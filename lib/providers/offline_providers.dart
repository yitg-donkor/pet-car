...
class PetsOffline extends _$PetsOffline {
  ...
  Future<List<Pet>> build() async {
    ...
    Future.microtask(() async {
      try {
        ...
        if (isOffline) {
          print('🟡 PetsOffline: Offline mode - skipping sync');
          return;
        }
  ...
  Future<void> addPet(Pet pet) async {
    print('🔵 PetsOffline: Starting addPet for ${pet.name}');

    // Get profile with fallback to local DB
    UserProfile? profile;

    try {
      profile = await ref.read(currentUserProfileProvider.future);
      print('✅ PetsOffline: Got profile from provider: ${profile?.id}');
    } catch (e) {
      print('⚠️ PetsOffline: Error getting profile from provider: $e');

      // Fallback: Try to get from local DB directly
      try {
        final profileLocalDB = ref.read(profileLocalDBProvider);
        final localProfiles = await profileLocalDB.getAllProfiles();

        if (localProfiles.isNotEmpty) {
          profile = localProfiles.firstWhere(
            (p) => p.isActive,
            orElse: () => localProfiles.first,
          );
          print(
            '✅ PetsOffline: Got profile from local DB fallback: ${profile.id}',
          );
        }
      } catch (localError) {
        print(
          '❌ PetsOffline: Failed to get profile from local DB: $localError',
        );
      }
    }

    if (profile == null) {
      print('❌ PetsOffline: No profile found - cannot add pet');
      throw Exception('User not logged in - please create a profile first');
    }

    print('🔵 PetsOffline: Adding pet ${pet.name} for user ${profile.id}');

    final petLocalDB = ref.read(petLocalDBProvider);
    final petWithOwner = Pet(
      id: '', // Will be generated by local DB
      ownerId: profile.id,
      name: pet.name,
      species: pet.species,
      breed: pet.breed,
    ...
    try {
      ...
      print('❌ PetsOffline: Failed to save pet locally: $e');
      throw Exception('Failed to save pet: $e');
    }
    ...
    if (!isOffline) {
      Future.microtask(() async {
        try {
          ...
          print('🔴 PetsOffline: Background sync error (not critical): $e');
          // Not critical - pet is already saved locally
        }
      });
    } else {
      print('🟡 PetsOffline: Offline mode - will sync when online');
    }
  }
